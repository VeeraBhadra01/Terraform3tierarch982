# Creating load balancer for web-alb
...
# Creating target groups for web tier
resource "aws_lb_target_group" "web-elb-tg" {
    name = var.tg-name
    health_check {
      enabled = true
      interval = 10
...
resource "aws_lb_listener" "web-elb-listener" {
    ...
    default_action {
      type = "forward"
      target_group_arn = aws_lb_target_group.web-elb-tg.arn
    }
